...
func TestTheBardSearch(t *testing.T) {
	...
	err = http.Serve(ln, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path == "/" || r.URL.Path == "/Shakespeare" {
			http.Redirect(w, r, "/shakespeare.mit.edu/index.html", http.StatusFound)
			return
		}
		...
		if r.URL.Path == "/searchresults.html" {
			...
			// render results page
			sres, err := os.ReadFile("testdata/searchresults.html")
			if err != nil {
				panic(err)
			}
		...
		if strings.HasSuffix(r.URL.Path, ".html") {
			...
			// render search form
			sf, err := os.ReadFile("testdata/searchform.html")
			if err != nil {
				panic(err)
			}
